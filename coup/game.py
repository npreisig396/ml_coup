import random
from .interfaces import Blank, Simple, Terminal
from .utils import Deck, Player, State

def main():
    g = Game([Simple() for i in range(4)],verbose=True)
    g.play()
...
class Game:
    ...
    def next_turn(self):
        self.history.append(tuple([tuple(player.cards) for player in self.players]))
    ...
    def offer_challenge(self,idx,msg,card,target=None): # Return 0 if succesful challenge, 1 if challenge failed, 2 if no challenge
        if target != None:
            if self.interfaces[target].get_challenge(State(self,target,action=card,actioner=idx)):
                if self.v:
            ...
            while i != idx:
                if self.players[i].cards and self.interfaces[i].get_challenge(State(self,i,action=card,actioner=idx)):
                    if self.v:
    ...
    def handle_challenge(self,challengee,challenger,card):
        i = self.interfaces[challengee].reveal_card(State(self,challengee,action=card))
        ...
        if c == card:
            ...
            c = self.players[challenger].cards.pop(i := self.interfaces[challenger].lose_influence(State(self,challenger)))
            self.deck.replace(c)
            if self.v:
    ...
    def offer_block(self,idx,msg,card,target=None):
        if target != None:
            if self.interfaces[target].get_block(State(self,target,action=card,actioner=idx)):
                if self.v:
            ...
            i = (idx+1) % self.num_players
            while i != idx:
                if self.players[i].cards and self.interfaces[i].get_block(State(self,i,action=card,actioner=idx)):
                    if self.v:
    ...
    def handle_block(self,blockee,blocker,card):
        if self.offer_challenge(blocker,f'Player {blocker} is blocking with {card}...',card,target=blockee):
            return 0
        else:
            return 1
    ...
    def assassinate(self): 
        self.players[self.idx].coins -= 3
        t = self.interfaces[self.idx].get_target(State(self,self.idx,action='Assassin'))
        if self.v:
            print(f'Player {self.idx} is assassinating player {t}')
        ch = self.offer_challenge(self.idx,f'Player{self.idx} is assassinating {t}', 'Assassin', target=t)
        if ch == 2:
    ...
    def steal(self):
        t = self.interfaces[self.idx].get_target(State(self,self.idx,action='Captain'))
        if self.v:
        ...
        if self.offer_challenge(self.idx,f'Player{self.idx} is stealing from {t}...', 'Captain', target=t):
            if self.players[t].cards:
                b = self.offer_block(self.idx,f'Player{self.idx} is stealing from {t}', 'Captain', target=t)
                if b == 2:
    ...
    def coup(self):
        ...
        t = self.interfaces[self.idx].get_target(State(self,self.idx,action=None))
        if self.v:
